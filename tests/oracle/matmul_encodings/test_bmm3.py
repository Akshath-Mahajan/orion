...
def test_bmm3_cipher_matches_numpy(ctx, nmpn):
    ...
    got = bmm3_he(ctx, A, B, n_he=n_he)
    want = A @ B

    assert got.shape == want.shape
    max_err = float(np.max(np.abs(got - want)))
    assert max_err < 1e-1, (
        f"BMM-III cipher result deviates from numpy.matmul by {max_err:.3e} "
        f"(n={n}, m={m}, p={p}, n_he={n_he}, "
        f"backend={type(ctx.backend).__name__})"
    )


def test_bmm3_cipher_matches_plaintext_oracle(ctx):
    ...
    max_err = float(np.max(np.abs(got - want)))
    assert max_err < 1e-1, (
        f"BMM-III cipher vs plaintext oracle: max_err={max_err:.3e} "
        f"(backend={type(ctx.backend).__name__})"
    )
